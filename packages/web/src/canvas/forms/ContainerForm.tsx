...
import { effectivePullPolicy, type PullPolicy } from "@hullbay/shared/node-config"
import { RailpackModal } from "../modals/RailpackModal"
...
export function ContainerForm({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="col-span-2">
          ...
          <Input
            ...
            placeholder={config.railpack ? "Chargée depuis GitHub" : "nginx ou ghcr.io/mon-org/mon-app"}
            disabled={Boolean(config.railpack)}
          />
          {config.railpack ? (
            <div className="mt-2 flex items-center gap-2 text-ui-fg-muted">
              <svg width="18" height="18" viewBox="0 0 16 16" fill="currentColor" aria-hidden>
                ...
                0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82a7.5 7.5 0 012.01-.27c.68 0 1.36.09 2.01.27
              ...
              <div>
                <div className="text-sm">Chargée depuis GitHub</div>
                <div className="text-xsmall text-ui-fg-muted">{config.railpack?.repoUrl}</div>
              </div>
            </div>
          ) : (
      ...
      <div className="mt-2">
        <Button size="small" variant="secondary" className="w-full justify-center" onClick={() => setRailOpen(true)}>
          <svg width="16" height="16" viewBox="0 0 16 16" className="mr-2" fill="currentColor" aria-hidden>
            <path fillRule="evenodd" d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38
            0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52
            -.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95
            0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82a7.5 7.5 0 012.01-.27c.68 0 1.36.09 2.01.27
      ...
      <RailpackModal
        ...
        initial={config.railpack ? { repoUrl: config.railpack.repoUrl, branch: config.railpack.branch } : undefined}
        onSave={(v) =>
          set({
            railpack: {
              repoUrl: v.repoUrl,
