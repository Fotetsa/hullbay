...
describe("expandPostgres HA (S4 §12-13)", () => {
  ...
  it("S4-11 : config membre passe le round-trip ContainerConfigSchema", () => {
    ...
    expect(parsed.tag).toBe("v1.2.4-pg16")
  ...
  it("version du contrat honorée en HA : le tag image suit la version MAJEURE (16.3 → v1.2.4-pg16)", () => {
    ...
    expect(member.config.tag).toBe("v1.2.4-pg16")
  })

  it("version majeure seule (17) → tag v1.2.4-pg17 ; pas de POSTGRESQL_VERSION runtime", () => {
    const exp = expandPostgres(cfg({ version: "17" }), ctx)
    const member = byRole(exp, "member")[0]!
    expect(member.config.tag).toBe("v1.2.4-pg17")
